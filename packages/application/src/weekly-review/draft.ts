/**
 * Weekly Review draft generator — canonical shared implementation.
 * Extracted from apps/web/src/lib/weekly-review-generator.ts and re-owned by
 * @ega/application so web and mobile share one source (no second source).
 */

export type WeeklyReviewTaskActivity = Readonly<{
  id: string;
  title: string;
  status: string;
...
export type WeeklyReviewTimeBucket = Readonly<{
  id: string;
  label: string;
  trackedSeconds: number;
  sessionCount: number;
...
export type WeeklyReviewDraftInput = Readonly<{
  weekStart: string;
  weekEnd: string;
...
export type WeeklyReviewDraft = Readonly<{
  summary: string;
  wins: string;
  blockers: string;
  nextSteps: string;
...
function formatTaskEstimate(minutes: number | null | undefined): string | null {
...
function sortByTrackedThenTitle<T extends { trackedSeconds: number; title?: string; label?: string }>(rows: T[]) {
  return [...rows].sort(
    (left, right) =>
...
function taskContext(task: WeeklyReviewTaskActivity): string {
  return [task.projectName, task.goalTitle].filter(Boolean).join(" / ");
}

function taskEstimateVsActual(task: WeeklyReviewTaskActivity): string | null {
  ...
  if (estimate && actual) return `estimate ${estimate}, tracked ${actual}`;
  if (estimate) return `estimate ${estimate}`;
  if (actual) return `tracked ${actual}`;
  return null;
}

function taskLine(task: WeeklyReviewTaskActivity): string {
  const details = [taskContext(task), taskEstimateVsActual(task)].filter(Boolean);
  return `${task.title}${details.length > 0 ? ` (${details.join("; ")})` : ""}`;
}

function topTaskLines(tasks: WeeklyReviewTaskActivity[]): string[] {
  return sortByTrackedThenTitle(tasks)
    .slice(0, LIST_LIMIT)
    .map(taskLine);
}

function topBlockedTaskLines(tasks: WeeklyReviewTaskActivity[]): string[] {
  return sortByTrackedThenTitle(tasks)
    .slice(0, LIST_LIMIT)
    .map((task) => {
...
function topBucketLines(buckets: WeeklyReviewTimeBucket[], fallbackLabel: string): string[] {
  return [...buckets]
    .sort(
      (left, right) =>
        right.trackedSeconds - left.trackedSeconds ||
    ...
    .map(
      (bucket) =>
        `${bucket.label}: ${formatDurationLabel(bucket.trackedSeconds)} across ${formatCount(bucket.sessionCount, "session")}`,
    )
...
function previousContext(previousReview: WeeklyReviewDraftInput["previousReview"]): string {
  if (!previousReview) {
...
export function generateWeeklyReviewDraft(input: WeeklyReviewDraftInput): WeeklyReviewDraft {
  ...
  const totalTrackedSeconds = input.projectTime.reduce((total, bucket) => total + bucket.trackedSeconds, 0);
  const hasActivity =
  ...
  const summaryLines = hasActivity
    ? [
        `Week ${input.weekStart} to ${input.weekEnd}: ${formatCount(input.completedTasks.length, "task")} completed, ${formatCount(input.carriedTasks.length, "task")} carried forward, ${formatCount(input.blockedTasks.length, "blocker")} active, ${formatDurationLabel(totalTrackedSeconds)} tracked.`,
        touchedProjects.length > 0
