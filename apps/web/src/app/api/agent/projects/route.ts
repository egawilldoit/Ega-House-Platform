// Production route handler for GET /api/agent/projects.
...
const telemetry: TelemetryFn = async (tokenId) => {
  after(async () => {
    try {
      ...
      console.warn(
        "[agent-projects] last_used_at update failed:",
...
export const { GET_PROJECTS: GET } = createReadHandlers(repo, rateLimiter, telemetry);
