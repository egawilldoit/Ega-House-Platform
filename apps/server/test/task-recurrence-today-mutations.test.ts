...
test("Today mutation endpoints plan, remove, update status, and clear completed", async () => {
  ...
  const a = app(fake);
  assert.equal((await a.request("/api/today/tasks/task-1",{method:"POST",headers:JSON_HEADERS,body:JSON.stringify({date:"2026-08-10"})})).status,200);
  assert.equal((await a.request("/api/today/tasks/task-1",{method:"DELETE",headers:AUTH})).status,200);
  assert.equal((await a.request("/api/today/tasks/task-1/status",{method:"PATCH",headers:JSON_HEADERS,body:JSON.stringify({status:"in_progress"})})).status,200);
  ...
  assert.deepEqual(await clear.json(),{ok:true,clearedCount:2});
});
